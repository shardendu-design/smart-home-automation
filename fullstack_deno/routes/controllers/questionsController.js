...
const addQuestion = async ({ request, response, render, state, params }) => {
  ...
  const showQuestionPage = async ({ params, render }) => {
    ...
    let questionDataArray = await questionService.getQuestionByQuestionID(id);
  
    const questionData = questionDataArray[0];
  
    const details = await questionAnswerService.getAnswerByQuestionId(id);
    ...
    render("question.eta", modifiedQuestionData);
  };
...
const getRandomQuestion = async ({ response, params }) => {
  ...
  if (randomQuestion) {
    ...
    response.body = "There are no questions";
  }
};
